...
contract EIP4337Manager is IAccount, GnosisSafeStorage, Executor {
    ...
    function executeAndRevert(address to, uint256 value, bytes memory data, Enum.Operation operation) external {
        ...
        // Adopted from:
        // https://github.com/Uniswap/v3-periphery/blob/464a8a49611272f7349c970e0fadb7ec1d3c1086/contracts/base/Multicall.sol#L16-L23
        if (!success) {
     ...
     * - this method is called with delegateCall, so the module (usually itself) is passed as parameter, and "this" is
     * the safe itself
     */
    ...
    function validateEip4337(GnosisSafe safe, EIP4337Manager manager) public {
        ...
        UserOperation memory userOp = UserOperation(address(safe), nonce, "", "", 0, 1_000_000, 0, 0, 0, "", sig);
